...
export function computeDrift(context = {}) {
  ...
  const matchesNode = (node, peer) => {
    const names = [node.hostname, node.device, node.transport?.tailscale_hostname]
      .map(normalizeHost)
      .filter(Boolean);
    return names.includes(normalizeHost(peer.hostname));
  };
  ...
  for (const node of catalogue.nodes || []) {
    ...
    const isObserver = normalizeHost(mesh.hostname) === normalizeHost(node.hostname);
    const peer = (mesh.peers || []).find(item => matchesNode(node, item));
    if (mesh.available && !isObserver && !peer) {
      drift.push({
        kind: "catalogue_vs_live",
...
export function nextActions(drift = [], summary = {}) {
  const actions = [];
  if (drift.some(item => item.kind === "aggregator_unreachable")) {
  ...
  for (const item of drift) {
    ...
    if (item.kind === "local_service_down") {
      ...
      if (fact.includes("agent-gateway")) {
        actions.push("Restore the local Agent Gateway service and verify its health authentication.");
      } else if (fact.includes("inox")) {
        actions.push("Restart InoxServeCapableHost scheduled task or inox-serve process.");
      } else if (fact.includes("ona self probe")) {
        actions.push("Inspect the Operium Node Agent supervisor and local health endpoint.");
      }
    }
  }
