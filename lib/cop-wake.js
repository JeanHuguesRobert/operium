...
export function obligationFromWakePacket(body, context = {}) {
  ...
  const packet = context.packet || wake.payload.packet;
  if (!packet?.payload && !wake.payload.packet_ref) {
    throw new Error("wake_requires_packet_or_packet_ref");
  }
  if (!packet?.payload) {
    throw new Error("packet_ref_unreadable");
  }
  const work = packet.payload || {};
