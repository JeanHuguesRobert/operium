...
export function formatNodeDiagnoseHuman(diagnose) {
  ...
  if (nodeAgent.reachable === false) {
    lines.push(`ONA         FAIL  ${nodeAgent.error || "unreachable"} @ ${nodeAgent.url || "?"}`);
  } else if (nodeAgent.ok === false) {
    lines.push(
      `ONA         DEGRADED  health=${nodeAgent.health_score ?? "?"} peers(fresh)=${nodeAgent.peer_count_fresh ?? 0} @ ${nodeAgent.url || "?"}`,
    );
    const probes = nodeAgent.probes || {};
    if (probes.passing?.length) {
      lines.push(`  ok:   ${probes.passing.join(", ")}`);
    }
    for (const item of probes.failing || []) {
      const status = item.http_status ? ` HTTP ${item.http_status}` : "";
      lines.push(`  FAIL: ${item.kind}${status} — ${item.error || "unknown error"}`);
    }
  } else {
    lines.push(
