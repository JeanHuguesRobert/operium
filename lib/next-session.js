...
export async function planNextSteps(options = {}) {
  ...
  if (issueHandle) {
    const cogentiaConnected = status.cogentia?.bridge?.available;
    const contCount = status.cogentia?.semantics?.continuations_count || 0;
    steps.push({
      priority: "P3 (Suite)",
      recommended: false,
      id: "advance_issue",
      ...
      reason: cogentiaConnected
        ? `Pont Cogentia connecté (${contCount} continuations dans le corpus) ; continuer les tâches jalonnées dans l'issue.`
        : "Dérouler les prochaines tâches prévues dans l'issue GitHub de cadrage.",
      impact: "Progresse vers la réalisation des objectifs de l'issue.",
    });
  }
