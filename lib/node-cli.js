...
import { recoverOna } from "./ona-recovery.js";
...
export async function runNodeCliCommand(options = {}) {
  switch (options.subcommand) {
    ...
    case "recover-ona":
      return { ok: true, body: await recoverOna({ host: options.invokeHost, registryPath: options.registryPath, env: options.env }) };
    default:
...
export async function runNodeDiagnoseCommand(options = {}) {
  ...
  if (options.recoverOna) {
    body.recovery = await recoverOna({
      host: options.invokeHost,
      registryPath: options.registryPath,
      env: options.env,
    });
    if (!body.recovery.ok) body.ok = false;
  }
  return { ok: true, body };
}
