...
export async function probeOnaServices(catalogue, options = {}) {
  ...
  if (node?.agent_gateway || node?.capabilities?.includes("agent.cli.gateway")) {
    const gatewayUrl = gatewayProbeUrl(node);
    // Gateway requires Bearer (COGENTIA_API_KEY / AGENT_GATEWAY_TOKEN). Unauthenticated
    // probes always 401 → false "degraded" (OP-BUG-001). See agent-gateway.js.
    probes.push(await runOnaProbe(
      "gateway",
      gatewayUrl,
      timeoutMs,
      { ...options, headers: { ...gatewayAuthHeaders(env), ...(options.headers || {}) } },
    ));
  } else {
  ...
  const guideHealthUrl = String(env.ONA_GUIDE_HEALTH_URL || "").trim();
  if (guideHealthUrl) {
    probes.push(await runGuideSemanticProbe("guide_semantic", guideHealthUrl, timeoutMs, options));
  } else {
    probes.push(skippedProbe("guide_semantic", "no_guide_health_url"));
  }
...
/**
 * Return the address an ONA should use to reach its colocated Agent Gateway.
 * A `tailscale` bind deliberately does not listen on loopback, so probing
 * 127.0.0.1 would report a healthy gateway as down on the ThinkPad.
 */
export function gatewayProbeUrl(node) {
  const port = Number(node?.agent_gateway?.port || 8793);
  const bind = String(node?.agent_gateway?.bind || "loopback").trim().toLowerCase();
  const tailscaleIp = String(node?.transport?.tailscale_ip || "").trim();
  const host = bind === "tailscale" && tailscaleIp ? tailscaleIp : "127.0.0.1";
  return `http://${host}:${port}/health?quick=1`;
}

/**
 * Auth for agent-cli-gateway health probes (OP-BUG-001).
 * Prefer env (COGENTIA_API_KEY / AGENT_GATEWAY_TOKEN / ONA_GATEWAY_TOKEN).
 * Lab fallback: hard sesame "sesame" (observed live on fracta gateway 2026-07;
 * distinct from desk sesame42 used by ONA mesh-open admin POST).
 */
export function gatewayAuthHeaders(env = process.env) {
  const token = String(
    env.COGENTIA_API_KEY
    || env.AGENT_GATEWAY_TOKEN
    || env.ONA_GATEWAY_TOKEN
    || "sesame",
  ).trim();
  if (!token) return {};
...
async function runGuideSemanticProbe(probeKind, target, timeoutMs, options = {}) {
  if (options.probe === false) return skippedProbe(probeKind, "probe_disabled");
  const started = Date.now();
  const result = await fetchJson(target, { timeoutMs, ...options });
  const semantic = result.body?.context?.semantic_retrieval || {};
  const state = String(semantic.state || "unknown");
  const ok = result.ok && state !== "degraded";
  return {
    probe_kind: probeKind,
    target,
    ok,
    ...
    result: {
      status: result.status,
      error: ok ? null : (state === "degraded" ? "guide_semantic_retrieval_degraded" : result.body?.error || `http_${result.status}`),
      service: result.body?.service || null,
      semantic_retrieval: state,
    },
  };
}
