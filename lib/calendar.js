...
export function normalizeObligation(raw = {}, context = {}) {
  ...
  return {
    ...
    calendar_origin: inferCalendarOrigin(raw),
    authorized: false,
...
function inferService(item = {}) {
  ...
  if (kind === "continuation" || kind.startsWith("continuation.")) return "continuation";
...
function summarize(items, fromJobs, fromObligations) {
  return {
    ...
    from_catalogue: items.filter(item => item.calendar_origin === "catalogue").length,
    from_wakes: items.filter(item => item.calendar_origin === "wake").length,
  };
}

function inferCalendarOrigin(raw = {}) {
  if (raw.calendar_origin === "catalogue" || raw.calendar_origin === "wake") {
    return raw.calendar_origin;
  }
  const source = String(raw.source_of_truth || "");
  if (source.startsWith("scheduled_jobs:")) return "catalogue";
  if (source.startsWith("cop/node.wake.v1:")) return "wake";
  if (raw.config?.wake) return "wake";
  const kind = String(raw.kind || "");
  if (kind === "ona.heartbeat") return "catalogue";
  return "wake";
}
