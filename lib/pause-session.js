...
function isSecretLikePath(filePath) {
  ...
  if (["id_rsa", "id_ed25519", "fractanet-mesh"].includes(base)) return true;
  if ([".pem", ".key", ".p12", ".pfx"].includes(ext)) return true;
  return /secret|token|credential/.test(normalized);
}
...
export async function pauseSession(options = {}) {
  ...
  // Handle continuation resumption if requested
  let resolvedContinuationId = null;
  let preloadedState = null;
  if (options.resumeContinuation || options.continuation) {
    const contId = options.resumeContinuation || options.continuation;
    const cont = loadContinuation(contId);
    if (!cont) {
      return {
        schema: "operium.pause_state.v1",
        ok: false,
        error: `continuation_not_found: ${contId}`,
      };
    }
    if (cont.status !== "active") {
      return {
        schema: "operium.pause_state.v1",
        ok: false,
        error: `continuation_already_${cont.status}: ${contId}`,
      };
    }
    resolvedContinuationId = contId;
    preloadedState = cont.context || {};
  }

  // Initial resolve issue & topic
  let canonicalIssueUrl = options.issue || options.choice || options.decision || null;
  const topic = options.topic || preloadedState?.topic || existingAnchor?.topic_id?.replace(/^topic:/, "") || "workspace-continuation";
  ...
  for (const { name, st } of statuses) {
    ...
    const ahead = st.sync?.ahead || 0;
    const behind = st.sync?.behind || 0;
    repoStatuses.push({
      name,
      ...
      ahead,
      behind,
      upstream: st.sync?.upstream || null,
      remote: st.repo?.remote || "origin",
    });
  }
  ...
  // 3. Synchronize any ahead repos to origin so anchored commits exist on remotes
  const reposPushed = [];
  const unpushedAlerts = [];

  if (!options.dryRun && options.push !== false) {
    for (const r of repoStatuses) {
      if (r.name === "JeanHuguesRobert") continue; // JHR anchor is committed and pushed below
      if (r.ahead > 0) {
        const rDir = path.resolve(WORKSPACE_ROOT, r.name);
        try {
          const pushArgs = r.upstream ? ["push"] : ["push", "-u", r.remote || "origin", r.branch];
          const pushRes = await runGit(pushArgs, { cwd: rDir, allowFailure: true });
          if (pushRes.ok) {
            reposPushed.push(r.name);
            r.ahead = 0;
          } else {
            unpushedAlerts.push({
              name: r.name,
              ahead: r.ahead,
              error: pushRes.stderr || pushRes.stdout || "push_failed",
            });
          }
        } catch (err) {
          unpushedAlerts.push({ name: r.name, ahead: r.ahead, error: err.message });
        }
      }
    }
  } else {
    for (const r of repoStatuses) {
      if (r.name !== "JeanHuguesRobert" && r.ahead > 0) {
        unpushedAlerts.push({ name: r.name, ahead: r.ahead, reason: "push_disabled_or_dry_run" });
      }
    }
  }
  ...
  // Resolve canonical issue or emit continuation if ambiguous
  if (!canonicalIssueUrl) {
    const inferred = inferSessionIssue(repoStatuses, existingAnchor, options);
    if (inferred.unambiguous && inferred.issue) {
      canonicalIssueUrl = inferred.issue;
    } else {
      // Choice Point: Emit cogentia.continuation.v2
      const continuation = emitChoicePointContinuation({
      ...
      return {
        ...
        repos_pushed: reposPushed,
        online_peers: onlinePeers,
        remotes_notified: remotesNotified,
        resume_hint: continuation.resume.command,
      };
    }
  }
  ...
  if (canonicalIssueUrl) {
    ...
    if (m) {
      issueHandle = `${m[2]}/${m[3]}`;
    } else if (/^[^/]+\/\d+$/.test(String(canonicalIssueUrl).trim())) {
      issueHandle = String(canonicalIssueUrl).trim();
      canonicalIssueUrl = `https://github.com/JeanHuguesRobert/${issueHandle.split("/")[0]}/issues/${issueHandle.split("/")[1]}`;
    }
  }
  ...
  if (!options.dryRun) {
    ...
    const newContent = [
      ...
      "",
      ...repoStatuses.map(r => {
        const syncSuffix = r.ahead > 0 ? ` (ahead ${r.ahead})` : "";
        return `- **\`${r.name}\`** : branch \`${r.branch}\` (head \`${r.head ? r.head.slice(0, 7) : "?"}\`) · clean: \`${r.clean}\`${syncSuffix}`;
      }),
      "",
    ...
    if (resolvedContinuationId) {
      resolveContinuation(resolvedContinuationId, {
        decision: canonicalIssueUrl,
        reason: "Session pause finalized with specified issue",
        packet_id: newPacketId,
      });
    }
  }
  ...
  const result = {
    ...
    resolved_continuation: resolvedContinuationId,
    repos_scanned: repoStatuses,
    repos_pushed: reposPushed,
    unpushed_alerts: unpushedAlerts,
    online_peers: onlinePeers,
    remotes_notified: remotesNotified,
...
export function formatPauseHuman(result) {
  if (result.status === "pending_continuation") {
    const lines = [
      "================================================================================",
      "⏸️  FRACTANET SESSION PAUSE: CHOICE POINT REQUIRED (Continuation Emitted)",
      "================================================================================",
      "",
      `❓ Question: ${result.question || "Which GitHub issue should anchor this session?"}`,
    ...
    for (let i = 0; i < (result.candidates || []).length; i++) {
      ...
      lines.push(`   ${i + 1}. ${c.handle.padEnd(20)} (${c.reason || c.url})`);
    }
    lines.push("");
    lines.push("👉 To resolve and complete suspension, resume the continuation with your choice:");
    lines.push(`   ${result.resume_hint || `operium pause --resume-continuation ${result.continuation_id} --issue <handle>`}`);
    lines.push("================================================================================\n");
    return lines.join("\n");
  }
  ...
  for (const r of repos) {
    ...
    const syncStr = r.ahead > 0 ? ` [ahead: ${r.ahead}]` : "";
    lines.push(`   - ${r.name.padEnd(20)}: ${r.branch.padEnd(20)} [${statusStr}]${syncStr}`);
  }

  if ((result.repos_pushed || []).length > 0) {
    lines.push(`\n🚀 Ahead Commits Pushed to Remote: ${result.repos_pushed.join(", ")}`);
  }
  if ((result.unpushed_alerts || []).length > 0) {
    lines.push(`\n⚠️  Unpushed Local Commits Detected:`);
    for (const u of result.unpushed_alerts) {
      lines.push(`   - ${u.name.padEnd(20)}: ${u.ahead} commit(s) ahead of remote (${u.error || u.reason || "unpushed"})`);
    }
  }
