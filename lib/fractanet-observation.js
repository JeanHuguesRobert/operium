...
export function buildDivergenceReport({ manifest, observations, reportedAt = new Date().toISOString() }) {
  ...
  for (const node of manifest.nodes) {
    ...
    if (node.expected_hostname && observed.observation.hostname !== node.expected_hostname) {
      divergences.push({
        node_id: node.node_id,
        invariant: "ssh_identity",
        expected: node.expected_hostname,
        observed: observed.observation.hostname,
