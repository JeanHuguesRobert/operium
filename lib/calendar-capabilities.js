...
import { applyContinuationResolve } from "./node-agent/cop-resolve.js";
...
export function resolveCalendar(deps, packet) {
  return applyContinuationResolve(deps.db, packet, {
    nodeId: deps.nodeId || deps.config?.nodeId,
    hostname: deps.hostname || deps.config?.hostname,
    now: deps.now,
  });
