  ...
  async function runDueJobs(options = {}) {
    ...
    for (const row of due) {
      ...
      `).run(
        finishedAt,
        result.ok ? 1 : 0,
        result.ok ? null : summarizeJobFailure(result),
        nextRun,
        finishedAt,
        job.job_id,
      );
    ...
    const calendar = await runDueCalendarObligations(db, options);
    return {
      ok: true,
      ran: results.length,
      results,
