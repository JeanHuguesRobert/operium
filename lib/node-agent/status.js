...
export function buildNodeStatus(deps = {}) {
  ...
  return {
    ...
    sqlite_stats: {
      ...
      calendar_obligations: countRows(db, "calendar_obligations"),
      cop_events: countRows(db, "cop_events"),
    },
    generated_at: new Date().toISOString(),
  };
}

function summarizeRecovery(db) {
  const activeCooldowns = db.prepare(`
    SELECT hostname, last_outcome, cooldown_until
    FROM ona_recovery_state
    WHERE cooldown_until > ?
    ORDER BY cooldown_until ASC
  `).all(new Date().toISOString());
  const receipts = db.prepare(`
    SELECT hostname, outcome, attempted, observed_at
    FROM ona_recovery_receipts
    ORDER BY observed_at DESC, rowid DESC
    LIMIT 10
  `).all();
  return {
    active_cooldowns: activeCooldowns,
    recent_receipts: receipts.map(row => ({ ...row, attempted: row.attempted === 1 })),
  };
}
