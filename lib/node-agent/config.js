...
export function loadOnaConfig(env = process.env) {
  ...
  return {
    ...
    // Tailscale-trust control room (La Nasa MIB-lite v0): open GET status/SOMA read
    // without bearer. Admin/COP POST still require tokens. Do not expose :8794 on WAN.
    // Default follows ONA_HEALTH_PUBLIC so mesh desk nodes already “public health”
    // also allow mesh-open read without a second flag (override with ONA_MESH_OPEN_READ=0).
    meshOpenRead: parseBooleanEnv(
      env.ONA_MESH_OPEN_READ,
      parseBooleanEnv(env.ONA_HEALTH_PUBLIC, bind === "127.0.0.1"),
    ),
    registryPath: String(env.OPERIUM_REGISTRY || "").trim() || defaultRegistryPath(env),
    // SNMP sysLocation / sysContact spirit (MIB-lite configuration scalars)
    location: String(env.ONA_LOCATION || env.ONA_SYS_LOCATION || "").trim() || null,
    contact: String(env.ONA_CONTACT || env.ONA_SYS_CONTACT || "").trim() || null,
