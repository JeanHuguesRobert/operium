...
const ATTRIBUTE_DEFINITIONS = Object.freeze({
  ...
  "core.location": {
    value_type: "String",
    behaviour_type: "Configuration",
    semantics: "Physical or organisational location of the managed node (SNMP sysLocation spirit)",
  },
  "core.contact": {
    value_type: "String",
    behaviour_type: "Configuration",
    semantics: "Operator contact for the managed node (SNMP sysContact spirit); typically an email",
  },
  "system.hostname": {
    value_type: "String",
...
const FACETS = Object.freeze({
  ...
  "core.identity": {
    attributes: ["core.user-label", "core.location", "core.contact"],
  },
  "system.runtime": {
    attributes: [
      "system.hostname",
...
export function buildSomaObject(deps = {}) {
  ...
  // SNMP sysLocation / sysContact (configuration; omit when unset)
  const location = config?.location ? String(config.location).trim() : "";
  const contact = config?.contact ? String(config.contact).trim() : "";
  if (location) attributes["core.location"] = location;
  if (contact) attributes["core.contact"] = contact;
  ...
  if (location || contact) facets.splice(1, 0, "core.identity");
