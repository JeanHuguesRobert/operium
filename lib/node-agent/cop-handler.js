...
import { listCopEvents } from "./cop-events.js";
...
import { listCalendar, resolveCalendar, scheduleCalendar } from "../calendar-capabilities.js";
import {
...
export async function handleCopHttpRequest(body, deps = {}) {
  ...
  if (envelope.packet_type === COP_NODE_PACKETS.RESOLVE && deps.adminAuth !== true) {
    return {
      status: 401,
      body: {
        ok: false,
...
const HANDLERS = {
  ...
  [COP_NODE_PACKETS.WAKE]: handleWakePacket,
  [COP_NODE_PACKETS.RESOLVE]: handleResolvePacket,
};
...
function handleQueryPacket(envelope, deps) {
  ...
  } else if (query === "cop_events") {
    data = {
      query,
      events: listCopEvents(deps.db, {
        kind: payload.kind,
...
function handleWakePacket(envelope, deps) {
  try {
    const scheduled = scheduleCalendar({
      db: deps.db,
      config: deps.config,
      nodeId: deps.nodeId,
    }, envelope);
    appendEventLog(deps.db, "cop/node.wake.v1", {
      source_node_id: envelope.sender?.node_id || null,
      cop_envelope_id: envelope.id,
      obligation_id: scheduled.obligation.id,
      authorized: false,
    });
    return {
      ok: true,
      handler: "wake",
      response_envelope: buildCopResponseEnvelope(envelope, scheduled, {
        nodeId: deps.nodeId,
        hostname: deps.config?.hostname,
      }),
    };
    ...
    return {
      ok: false,
      handler: "wake",
      response_envelope: buildCopErrorEnvelope(envelope, "wake_rejected", {
        nodeId: deps.nodeId,
        hostname: deps.config?.hostname,
        message: error.message || "wake_rejected",
      }),
    };
  }
}

function handleResolvePacket(envelope, deps) {
  try {
    ...
    appendEventLog(deps.db, "cop/node.resolve.v1", {
      source_node_id: envelope.sender?.node_id || null,
      cop_envelope_id: envelope.id,
      obligation_id: resolved.obligation.id,
      authorized: false,
    ...
    return {
      ok: true,
      handler: "resolve",
      response_envelope: buildCopResponseEnvelope(envelope, resolved, {
        nodeId: deps.nodeId,
        hostname: deps.config?.hostname,
      }),
    };
    ...
    return {
      ok: false,
      handler: "resolve",
      response_envelope: buildCopErrorEnvelope(envelope, error.message || "resolve_rejected", {
        nodeId: deps.nodeId,
        hostname: deps.config?.hostname,
        message: error.message || "resolve_rejected",
      }),
    };
  }
}
