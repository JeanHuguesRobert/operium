...
import { dnsObservationWakePacket, obligationFromWakePacket, parseWakePacket } from "../cop-wake.js";
import { appendCopEvent, COP_CALENDAR_KINDS } from "./cop-events.js";
import { getCopPacket, putCopPacket } from "./cop-packets.js";
...
export function upsertWakePacket(db, body, context = {}) {
  ...
  const now = context.now || new Date().toISOString();
  if (parsed.envelope.payload.packet) {
    putCopPacket(db, parsed.envelope.payload.packet, { now });
  }
  const packet = resolveInnerPacket(db, parsed.envelope.payload);
  if (!packet) {
    const error = new Error("packet_ref_unreadable");
    error.code = "packet_ref_unreadable";
    throw error;
  }
  const obligation = obligationFromWakePacket(body, { ...context, packet });
  appendCopEvent(db, {
    ...parsed.envelope,
    id: body.id || parsed.envelope.id || `cop:wake:${obligation.id}`,
    packet_type: COP_CALENDAR_KINDS.WAKE,
    created_at: body.created_at || parsed.envelope.created_at || now,
    payload: parsed.envelope.payload,
  }, {
    obligation_id: obligation.id,
    now,
  });
  return upsertCalendarObligation(db, obligation, context);
}

export function resolveInnerPacket(db, payload = {}) {
  if (payload.packet && typeof payload.packet === "object" && payload.packet.payload) {
    return payload.packet;
  }
  return getCopPacket(db, payload.packet_ref);
}
...
export function applyObligationStatus(db, id, update = {}) {
  const existing = getCalendarObligation(db, id);
  if (!existing) throw new Error(`unknown_calendar_obligation:${id}`);
  if (existing.status === "closed") return existing;
  const now = update.now || new Date().toISOString();
  const closed = Boolean(update.closed) || update.status === "closed";
  const status = closed ? "closed" : (update.status || existing.status);
  const evidence = update.evidence === undefined
  ...
  `).run(
    status,
    closed ? null : (update.next_run_at ?? existing.next_run_at),
    json(evidence),
    now,
    closed ? (update.closed_at || now) : existing.closed_at,
    id,
