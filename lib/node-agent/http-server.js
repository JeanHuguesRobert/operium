...
export function createOnaHttpServer(deps = {}) {
  ...
  async function handleRequest(req, res) {
    ...
    if (req.method === "POST" && url.pathname === "/node/calendar/schedule") {
      if (!hasAdminAuth(req, config)) {
        return sendJson(res, 401, { ok: false, error: "unauthorized_admin" });
      }
      const payload = await readJsonBody(req);
      try {
        const body = scheduleCalendar({
          config,
          db,
          nodeId: getNodeId(),
        }, payload);
        ...
        const code = String(error.code || error.message || "schedule_rejected");
        if (code === "packet_ref_unreadable" || code === "wake_requires_packet_or_packet_ref" || code === "not_a_wake_packet") {
          return sendJson(res, 400, { ok: false, error: code, message: error.message || code });
        }
        throw error;
      }
    }
    ...
    if (req.method === "POST" && url.pathname === "/node/cop") {
      ...
      const adminAuth = hasAdminAuth(req, config);
      const packetType = String(body.packet_type || body.type || "").trim();
      if (packetType === COP_NODE_PACKETS.RESOLVE && !adminAuth) {
