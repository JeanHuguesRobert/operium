...
// Lab desk sesame (Tailscale trust plane). Same spirit as Sesame42 on cogentia gateway.
// Accepted only when meshOpenRead is on — not a substitute for WAN auth.
export const MESH_DESK_SESAME = "sesame42";

function hasAdminAuth(req, config) {
  if (hasBearerToken(req, config.tokens.admin)) return true;
  // Mesh desk: allow hard sesame for SOMA actions / admin GETs while on trust perimeter
  if (config.meshOpenRead && hasBearerToken(req, MESH_DESK_SESAME)) return true;
  // Also accept x-ona-sesame / query for CGI simplicity (still only if meshOpenRead)
  if (config.meshOpenRead) {
    const headerSesame = String(req.headers?.["x-ona-sesame"] || "").trim();
    if (headerSesame && headerSesame === MESH_DESK_SESAME) return true;
  }
  return false;
}
