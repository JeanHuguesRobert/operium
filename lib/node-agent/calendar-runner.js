...
function handlerKey(obligation) {
  ...
  if (
    packetKind.startsWith("cogentia.") ||
    workKind.startsWith("cogentia.") ||
    workKind.startsWith("corpus.") ||
    packetKind.startsWith("corpus.")
  ) {
    return "cogentia.job";
  }
  return workKind;
...
async function runCogentiaWake(obligation, options = {}) {
  const work = obligation.config?.wake?.payload?.packet?.payload || obligation.config || {};
  return executeCogentiaWake({
    job: work.job || work.job_name || obligation.kind || "cogentia.wake",
    obligation_id: obligation.id,
    work,
  }, options);
}
