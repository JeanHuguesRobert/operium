...
import { backfillWakeEventsFromObligations } from "./calendar-replay.js";
...
export function openNodeMemoryDb(options = {}) {
  ...
  let backfill = { ok: true, created: 0, scanned: 0 };
  if (options.backfillCopEvents !== false) {
    backfill = backfillWakeEventsFromObligations(db, {
      now: options.now,
    });
  }

  return { db, dbPath, migration, backfill };
}
