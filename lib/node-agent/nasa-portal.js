...
// Keep in sync with MESH_DESK_SESAME in http-server.js (avoid circular import)
const DESK_SESAME = "sesame42";
...
export const FLEET_NODES = Object.freeze([
  ...
  { host: "rpi3-view", resource_id: "resource://rpi3-view", label: "This Pi" },
...
export async function handleNasaPortal(req, res, url, deps = {}) {
  ...
  // Mesh desk surface is open when mesh-open-read (Tailscale trust), same as status/SOMA GET
  const open = Boolean(config.meshOpenRead) || config.bind === "127.0.0.1";

  const p = url.pathname;

  // --- JSON APIs (compat + modern) ---
  if (req.method === "GET" && (p === "/cgi-bin/fleet" || p === "/nasa/fleet")) {
    if (!open) return sendJson(res, 401, { ok: false, error: "unauthorized" });
  ...
  if (req.method === "GET") {
    ...
    if (staticPath) {
      if (!open && p !== "/boot.html") {
        // allow boot splash only when open; otherwise still need mesh open for desk
      }
      if (!open) return sendJson(res, 401, { ok: false, error: "unauthorized" });
...
async function probeFleetNode(def, config) {
  ...
  const r = await fetchJson(endpoint, { timeoutMs: 5000 });
...
export async function buildNodePack(host, config, deps = {}) {
  ...
  const [objectR, vocabR, obsR, actionsR] = await Promise.all([
...
async function runNasaAction(host, name, config, deps) {
  ...
  const sesame = String(process.env.ONA_MESH_DESK_SESAME || DESK_SESAME).trim() || DESK_SESAME;
  const url = `${base}/soma/actions/${name}`;
