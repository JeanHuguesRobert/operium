...
function buildCatalogueJobs({ node, hostname, intervalMs, env }) {
  const jobs = [];
  if (!node) return jobs;
  const recovery = node.recovery_observer;
  if (recovery?.enabled && Array.isArray(recovery.targets)) {
    jobs.push({
      job_id: "recovery:ona-observer",
      kind: "script",
      interval_ms: Number(recovery.cadence_ms || 300000),
      enabled: true,
      config: {
        script: "operium/scripts/ops/recover-ona-observer.js",
        cwd: "operium",
        args: recovery.targets,
      },
    });
  }
