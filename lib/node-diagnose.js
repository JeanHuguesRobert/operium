...
function buildNodeAgentLayer(onaStatus, onaBase) {
  ...
  return {
    ...
    probes: summarizeProbes(body.probes),
  };
}

// "Degraded" is not actionable on its own — an operator needs to know which
// probe is failing and why, not just a health number (operational-health.md:
// "make fragility visible, discussable and actionable").
function summarizeProbes(probes) {
  const items = Array.isArray(probes?.items) ? probes.items : [];
  const applicable = items.filter(item => !item.skipped);
  const passing = applicable.filter(item => item.ok === true).map(item => item.probe_kind);
  const failing = applicable
    .filter(item => item.ok !== true)
    .map(item => ({
      kind: item.probe_kind,
      target: item.target || null,
      http_status: item.result?.status ?? null,
      error: item.result?.error?.message || item.result?.error || null,
    }));
  return { applicable: applicable.length, passing, failing };
}
...
function buildDiagnoseHeadline(operiumSummary, nodeAgent) {
  ...
  if (nodeAgent.reachable === false) return `${base} · ONA unreachable`;
  if (nodeAgent.ok === false || (nodeAgent.health_score != null && nodeAgent.health_score < 3)) {
    const failing = (nodeAgent.probes?.failing || []).map(item => item.kind);
    return failing.length ? `${base} · ONA degraded (${failing.join(", ")})` : `${base} · ONA degraded`;
  }
  return base;
}