...
| **P5** | Actions, history, richer graph; **optional re-auth** if perimeter expands | **partial 2026-07-30** — La Nasa zoom shows SOMA action catalogue + recent history (read-only). `/cgi-bin/node` proxies `/soma/actions`. **No execute buttons** yet (POST still needs admin token). Open: observation.refresh from desk, richer graph |
