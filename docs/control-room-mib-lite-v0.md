...
| **P5** | Actions, history, richer graph; **optional re-auth** if perimeter expands | **partial 2026-07-30** — La Nasa zoom: action catalogue + **Run** via `/cgi-bin/action`. Mesh desk auth = hard sesame **`sesame42`** when `ONA_MESH_OPEN_READ=1` (Tailscale trust; same spirit as gateway Sesame42). Real `ONA_ADMIN_TOKEN` still works. Open: richer graph, upgrade action |
