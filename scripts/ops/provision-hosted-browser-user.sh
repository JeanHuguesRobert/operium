...
if "$dry_run"; then
  ...
  plan "enable hosted-browser-health@${unix_user}.timer"
  "$with_rfb" && plan "enable localhost-only hosted-browser-rfb@${unix_user}.service"
  exit 0
fi

for required in \
  /opt/operium/bin/start-hosted-browser.sh \
  /opt/operium/bin/check-hosted-browser-health.sh \
  /etc/systemd/system/hosted-browser@.service \
  /etc/systemd/system/hosted-browser-health@.service \
  /etc/systemd/system/hosted-browser-health@.timer; do
...
systemctl enable --now "hosted-browser-health@${unix_user}.timer"
if "$with_rfb"; then
